...
import com.datamaster.module.ai.dal.dataobject.skill.AiSkillVersionDO;
import com.datamaster.module.ai.dal.mapper.skill.AiSkillMapper;
import com.datamaster.module.ai.dal.mapper.skill.AiSkillRefMapper;
import com.datamaster.module.ai.dal.mapper.skill.AiSkillVersionMapper;
...
public class AiSkillServiceImpl implements IAiSkillService {
    ...
    public AiSkillRespVO generateTableSkill(AiTableSkillGenerateReqVO reqVO) {
        ...
        DatasourceDO datasource = asset.getDatasourceId() == null ? null : datasourceMgmtService.getDatasourceDOById(asset.getDatasourceId());
        QualitySummaryRespDTO qualitySummary = findQualitySummary(asset);
    ...
    private QualitySummaryRespDTO findQualitySummary(AssetsAssetDO table) {
        if (table == null || table.getDatasourceId() == null || StringUtils.isBlank(table.getTableName())) {
            return null;
        }
        try {
            return qualityTaskApiService.getLatestQualitySummary(table.getDatasourceId(), table.getTableName());
    ...
    private void refreshTableRefs(AiSkillDO skill, AssetsAssetDO asset, List<AssetsAssetColumnDO> columns, DatasourceDO datasource) {
    ...
    private void refreshDatabaseRefs(AiSkillDO skill, DatasourceDO datasource, List<AssetsAssetDO> assets) {
        aiSkillRefMapper.deleteBySkillId(skill.getId());
        insertRef(skill.getId(), "DATA_SOURCE", datasource.getId(), datasource.getDatasourceType(), datasource.getDatasourceName());
        if (assets != null) {
            for (AssetsAssetDO asset : assets) {
    ...
    private void refreshMultiTableRefs(AiSkillDO skill, DatasourceDO datasource, List<AssetsAssetDO> assets,
    ...
    private SkillEnhancement generateTableEnhancement(AssetsAssetDO asset, DatasourceDO datasource,
    ...
    private String tableEnhancementPrompt(AssetsAssetDO asset, DatasourceDO datasource,
                                          List<AssetsAssetColumnDO> columns, String manualNotes) {
    ...
    private String tableSkillCode(DatasourceDO datasource, AssetsAssetDO asset) {
    ...
    private String databaseSkillCode(DatasourceDO datasource) {
        List<String> parts = new ArrayList<>();
        parts.add("database");
        parts.add(firstNonBlank(datasource.getDatasourceName(), datasource.getDatasourceType(), String.valueOf(datasource.getId())));
        return normalizeCode(join(parts, "-"));
    ...
    private String multiTableSkillCode(DatasourceDO datasource, List<AssetsAssetDO> assets) {
        List<String> parts = new ArrayList<>();
    ...
    private String tableSkillContent(String skillCode, AssetsAssetDO asset, DatasourceDO datasource,
                                     List<AssetsAssetColumnDO> columns, String manualNotes, SkillEnhancement enhancement,
                                     QualitySummaryRespDTO qualitySummary) {
    ...
    private String renderQualitySummary(QualitySummaryRespDTO summary) {
        if (summary == null) {
        ...
        if (summary.getRules() != null && !summary.getRules().isEmpty()) {
            ...
            builder.append("| --- | --- | --- | --- | --- | --- | --- | --- |\n");
            for (QualitySummaryRespDTO.Rule rule : summary.getRules()) {
                builder.append("| ")
