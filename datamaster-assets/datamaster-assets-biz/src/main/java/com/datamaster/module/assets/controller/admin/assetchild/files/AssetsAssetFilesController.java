package com.datamaster.module.assets.controller.admin.assetchild.files;
...
import com.datamaster.module.assets.convert.assetchild.files.AssetsAssetFilesConvert;
...
/**
 * 数据资产-文件服务Controller * * @author DATAMASTER * @date 2025-06-26
 */
@Tag(name = "数据资产-文件服务")
@RestController
@RequestMapping("/ast/assetFiles")
@Validated
public class AssetsAssetFilesController extends BaseController {
    @Resource
    ...
    public CommonResult<PageResult<AssetsAssetFilesRespVO>> list(AssetsAssetFilesPageReqVO AssetsAssetFiles) {
        ...
        return CommonResult.success(BeanUtils.toBean(page, AssetsAssetFilesRespVO.class));
    }
    ...
    public void export(HttpServletResponse response, AssetsAssetFilesPageReqVO exportReqVO) {
        ...
        List<AssetsAssetFilesDO> list = (List<AssetsAssetFilesDO>) AssetsAssetFilesService.getAssetFilesPage(exportReqVO).getRows();
        ExcelUtil<AssetsAssetFilesRespVO> util = new ExcelUtil<>(AssetsAssetFilesRespVO.class);
        util.exportExcel(response, AssetsAssetFilesConvert.INSTANCE.convertToRespVOList(list), "应用管理数据");
    }
    ...
    public AjaxResult importData(MultipartFile file, boolean updateSupport) throws Exception {
        ExcelUtil<AssetsAssetFilesRespVO> util = new ExcelUtil<>(AssetsAssetFilesRespVO.class);
        List<AssetsAssetFilesRespVO> importExcelList = util.importExcel(file.getInputStream());
        String operName = getUsername();
        String message = AssetsAssetFilesService.importAssetFiles(importExcelList, updateSupport, operName);
    ...
    public CommonResult<AssetsAssetFilesRespVO> getInfo(@PathVariable("id") Long id) {
        ...
        return CommonResult.success(BeanUtils.toBean(AssetsAssetFilesDO, AssetsAssetFilesRespVO.class));
    }
    ...
    public CommonResult<Long> add(@Valid @RequestBody AssetsAssetFilesSaveReqVO AssetsAssetFiles) {
        AssetsAssetFiles.setCreatorId(getUserId());
        AssetsAssetFiles.setCreateBy(getNickName());
        AssetsAssetFiles.setCreateTime(DateUtil.date());
        return CommonResult.toAjax(AssetsAssetFilesService.createAssetFiles(AssetsAssetFiles));
    }
    ...
    public CommonResult<Integer> edit(@Valid @RequestBody AssetsAssetFilesSaveReqVO AssetsAssetFiles) {
        AssetsAssetFiles.setUpdatorId(getUserId());
        AssetsAssetFiles.setUpdateBy(getNickName());
        AssetsAssetFiles.setUpdateTime(DateUtil.date());
        return CommonResult.toAjax(AssetsAssetFilesService.updateAssetFiles(AssetsAssetFiles));
    }
    ...
    public CommonResult<Integer> remove(@PathVariable Long[] ids) {
        return CommonResult.toAjax(AssetsAssetFilesService.removeAssetFiles(Arrays.asList(ids)));
    }
}