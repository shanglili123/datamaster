...
public class DbGptDatasourceSyncServiceImpl implements IDbGptDatasourceSyncService {
    ...
    public AjaxResult syncAll() {
        int success = 0;
        int failed = 0;
        List<AssetsDatasourceDO> datasources = assetsDatasourceMapper.selectList();
        for (AssetsDatasourceDO datasource : datasources) {
            try {
    ...
    public AjaxResult removeFromDbGpt(Long datasourceId) {
        ...
        if (datasource.getDbgptDatasourceId() != null) {
            dbGptClientService.deleteDatasource(datasource.getDbgptDatasourceId());
        }
        datasource.setDbgptDatasourceId(null);
        datasource.setDbgptSyncStatus("REMOVED");
        datasource.setDbgptSyncMessage("已从DB-GPT删除");
        assetsDatasourceMapper.updateById(datasource);
        return AjaxResult.success("已从DB-GPT删除数据源");
    }

    private Integer syncDatasource(AssetsDatasourceDO datasource) {
        ...
        datasource.setDbgptDatasourceId(remoteId);
    ...
    private Map<String, Object> buildParams(AssetsDatasourceDO datasource) {
    ...
    private void markFailed(AssetsDatasourceDO datasource, String message) {
