package com.datamaster.module.assets.utils;import com.alibaba.fastjson2.JSON;import org.springframework.beans.factory.annotation.Value;import org.springframework.stereotype.Component;import com.datamaster.api.ds.api.etl.DsSchedulerSaveReqDTO;import com.datamaster.api.ds.api.etl.DsSchedulerUpdateReqDTO;import com.datamaster.api.ds.api.etl.DsStartTaskReqDTO;import com.datamaster.api.ds.api.etl.DsTaskSaveReqDTO;import com.datamaster.api.ds.api.etl.ds.ProcessDefinition;import com.datamaster.api.ds.api.etl.ds.TaskDefinition;import com.datamaster.common.exception.ServiceException;import com.datamaster.common.utils.JSONUtils;import com.datamaster.module.assets.utils.model.TaskSaveReqInput;import java.text.SimpleDateFormat;import java.time.LocalDate;import java.time.format.DateTimeFormatter;import java.util.*;@Component
public class AssetsTaskConverter {    private static String defaultURL;    @Value("${ds.http_url:}")    private void setDefaultURL(String defaultURL) {        this.defaultURL = defaultURL;    }    private static final String DEFAULT_CONDITION_TYPE = "NONE";
...
// 默认标志，表示节点启用
// 默认配置常量
    private static final long DEFAULT_ENVIRONMENT_CODE = 133155949418208L;

// 默认环境编码
    private static final String DEFAULT_WORKER_GROUP = "default";

// 默认工作组
    private static final String DEFAULT_IS_CACHE = "NO";
...
// 1. 封装基础参数
    dsTaskSaveReqDTO.setName(input.getName());

// 任务名
    dsTaskSaveReqDTO.setDescription("");

// 描述为空
    dsTaskSaveReqDTO.setExecutionType("PARALLEL");
...
// 3. 构建任务关系
    String taskRelation = buildTaskRelationJson(input);
...
// 设置 HTTP 类型任务的参数
    ...
    taskMap.put("id", input.getNodeId());

// 使用传入的任务 ID
    taskMap.put("name", input.getName());

// 写死任务名
    taskMap.put("code", input.getNodeCode());

// 使用传入的任务 code
    taskMap.put("version", 1);

// 写死版本
    taskMap.put("description", "");

// 描述为空
    taskMap.put("workerGroup",DEFAULT_WORKER_GROUP);
...
// 不缓存
    taskMap.put("taskPriority", DEFAULT_TASK_PRIORITY);

// 中等优先级
    taskMap.put("taskType", DEFAULT_TASK_TYPE);
...
// 默认空列表
    taskParams.put("httpMethod", HTTP_METHOD);

// 默认 HTTP 方法为 GET
    taskParams.put("httpBody", HTTP_BODY);

// 默认 HTTP body 空
    taskParams.put("httpCheckCondition", HTTP_CHECK_CONDITION);

// 默认检查条件
    taskParams.put("httpParams", input.getHttpParams());

// 使用传入的 httpParams
    taskParams.put("url", defaultURL + "/"+ String.valueOf(input.getId()));

// 默认 URL
    taskParams.put("condition", CONDITION);

// 默认条件为空
    taskParams.put("connectTimeout", CONNECT_TIMEOUT);

// 默认连接超时 60000ms
    taskParams.put("socketTimeout", SOCKET_TIMEOUT);

// 默认 socket 超时 60000ms
...
// 构建任务关系
    ...
    relationMap.put("id", input.getNodeId());

// 写死关系 ID
    relationMap.put("preTaskCode", 0L);

// 前任务 code 写死为 0
    relationMap.put("preTaskVersion", 0);

// 前任务版本号写死为 0
    relationMap.put("postTaskCode", input.getNodeCode());

// 后任务 code 写死
    relationMap.put("postTaskVersion", 1);

// 后任务版本号写死
    relationMap.put("conditionType", DEFAULT_CONDITION_TYPE);
...
// 写死任务节点位置
    ...
    locationMap.put("taskCode", input.getNodeCode());

// 任务 ID
    locationMap.put("x", 138.4886474609375);

// 写死坐标 X
    locationMap.put("y", 184.9232940673828);
...
// 如果列表为空，返回 null 或根据需要返回其他默认值
    ...
    /***     *  DsSchedulerSaveReqDTO     *     * @param crontab Cron      * @param processDefinitionCode      * @return DsSchedulerSaveReqDTO     */
...
// 创建 DsSchedulerSaveReqDTO 并设置默认值
    ...
    /***     *  DsSchedulerUpdateReqDTO     *     * @param id ID     * @param crontab Cron      * @param processDefinitionCode      * @return DsSchedulerUpdateReqDTO     */
...
// 使用 builder 模式创建 DsStartTaskReqDTO 对象，其他字段均为写死的值
    return DsStartTaskReqDTO.builder()                .processDefinitionCode(JSONUtils.convertToLong(processDefinitionCode))                .failureStrategy("CONTINUE")                .warningType("NONE")                .processInstancePriority("MEDIUM")                .scheduleTime(scheduleTime)                .build();
    }}