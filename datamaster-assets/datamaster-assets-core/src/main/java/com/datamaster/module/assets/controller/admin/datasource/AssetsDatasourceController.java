...
/**
 * 数据源Controller * * @author lhs * @date 2025-01-21
 */
@Tag(name = "数据源")
@RestController
@RequestMapping("/ast/dataSource")
@Validated
...
public class AssetsDatasourceController extends BaseController {
    ...
    @Operation(summary = "数据集成中排除Kafka并且是当前空间的数据源列表")
    @PreAuthorize("@ss.hasPermi('ast:dataSource:list')")
    ...
    public AjaxResult dppNoKafkaList(AssetsDatasourcePageReqVO AssetsDatasource) {
        ...
        return AjaxResult.success(BeanUtils.toBean(page, AssetsDatasourceRespVO.class));
    }
    ...
    public CommonResult<PageResult<SpaceRespDTO>> noDppAddList(SpaceReqDTO pageReqVO) {
        PageResult<SpaceRespDTO> page = AssetsDatasourceService.getNoDppAddList(pageReqVO);
    ...
    public AjaxResult getDataSourceByAsset(AssetsDatasourceRespVO AssetsAsset) {
        List<AssetsDatasourceDO> AssetsAssetDOS = AssetsDatasourceService.getDataSourceByAsset(AssetsAsset);
        return AjaxResult.success(AssetsAssetDOS);
    }
    ...
    @PreAuthorize("@ss.hasPermi('ast:dataQuery:list')")
    ...
    public void export(HttpServletResponse response, AssetsDatasourcePageReqVO exportReqVO) {
        ...
        List<AssetsDatasourceDO> list = (List<AssetsDatasourceDO>) AssetsDatasourceService.getDatasourcePage(exportReqVO).getRows();
        ExcelUtil<AssetsDatasourceRespVO> util = new ExcelUtil<>(AssetsDatasourceRespVO.class);
        util.exportExcel(response, AssetsDatasourceConvert.INSTANCE.convertToRespVOList(list), "应用管理数据");
    }
    ...
    public AjaxResult importData(MultipartFile file, boolean updateSupport) throws Exception {
        ExcelUtil<AssetsDatasourceRespVO> util = new ExcelUtil<>(AssetsDatasourceRespVO.class);
        List<AssetsDatasourceRespVO> importExcelList = util.importExcel(file.getInputStream());
        String operName = getUsername();
        String message = AssetsDatasourceService.importDatasource(importExcelList, updateSupport, operName);
    ...
    public CommonResult<AssetsDatasourceRespVO> getInfo(@PathVariable("id") Long id) {
        ...
        return CommonResult.success(BeanUtils.toBean(AssetsDatasourceDO, AssetsDatasourceRespVO.class));
    }
    ...
    public CommonResult<Long> add(@Valid @RequestBody AssetsDatasourceSaveReqVO AssetsDatasource) {
        AssetsDatasource.setCreatorId(getUserId());
        AssetsDatasource.setCreateBy(getNickName());
        AssetsDatasource.setCreateTime(DateUtil.date());
        return CommonResult.toAjax(AssetsDatasourceService.createDatasource(AssetsDatasource));
    }
    ...
    public CommonResult<Integer> edit(@Valid @RequestBody AssetsDatasourceSaveReqVO AssetsDatasource) {
        AssetsDatasource.setUpdatorId(getUserId());
        AssetsDatasource.setUpdateBy(getNickName());
        AssetsDatasource.setUpdateTime(DateUtil.date());
        return CommonResult.toAjax(AssetsDatasourceService.updateDatasource(AssetsDatasource));
    }
    ...
    public CommonResult<Integer> remove(@PathVariable Long[] ids) {
        return CommonResult.toAjax(AssetsDatasourceService.removeDatasource(Arrays.asList(ids)));
    }
    ...
    public CommonResult<Integer> removeDppOrDa(@PathVariable("ids") Long[] ids, @PathVariable("type") Long type) {
        return CommonResult.toAjax(AssetsDatasourceService.removeDatasourceDppOrDa(Arrays.asList(ids), type));
    }
    ...
    @PreAuthorize("@ss.hasAnyPermi('ast:dataSource:query,ast:dataQuery:list')")
    ...
    public AjaxResult sqlParse(@RequestBody JSONObject jsonObject) {
        ...
        }       /* try {            jsonObject.set("sql",AesEncryptUtil.desEncrypt(jsonObject.getStr("sql")).trim());        } catch (Exception e) {            e.printStackTrace();        }*/
        List<DbColumn> dbColumnList = AssetsDatasourceService.sqlParse(jsonObject.getStr("sourceId"), jsonObject.getStr("sql"));
        List<AssetsAssetColumnDO> AssetsAssetColumnDOList = AssetsDatasourceServiceImpl.convertDbColumns(dbColumnList);
        return AjaxResult.success(AssetsAssetColumnDOList);
    }
    ...
    public CommonResult<List<AssetsAssetColumnDO>> columnsAsAssetColumnList(@RequestBody @Valid AssetsDatasourceTableVO param) {
        ...
        List<AssetsAssetColumnRelRuleVO> assetColumnRelRuleVOS = AssetsAssetService.listRelRule(param.getId(), param.getTableName(), param.getWithRule().toString());
        if (assetColumnRelRuleVOS.isEmpty()) {
            return CommonResult.success(columns);
        }
        columns.forEach(i -> i.setCleanRuleList(new ArrayList<>()));
        for (AssetsAssetColumnRelRuleVO vo : assetColumnRelRuleVOS) {
    ...
    public AjaxResult createTaskTempTable(@RequestBody JSONObject jsonObject) {
        ...
        dto.setDatasourceType(jsonObject.getStr("datasourceType"));
        dto.setDatasourceConfig(jsonObject.getStr("datasourceConfig"));
        dto.setIp(jsonObject.getStr("ip"));
        dto.setPort(jsonObject.getLong("port"));
        dto.setDbname(jsonObject.getStr("dbname"));
        dto.setTableName(jsonObject.getStr("tableName"));
        dto.setTableComment(jsonObject.getStr("tableComment"));
        JSONArray columns = jsonObject.getJSONArray("columns");
        List<DbColumn> dbColumns = new ArrayList<>();
        if (columns != null && !columns.isEmpty()) {
            for (int i = 0; i < columns.size(); i++) {
                JSONObject col = columns.getJSONObject(i);
                dbColumns.add(DbColumn.builder().colName(col.getStr("columnName")).dataType(col.getStr("columnType")).dataLength(col.getStr("columnLength")).dataScale(col.getStr("columnScale")).colKey("1".equals(col.getStr("pkFlag"))).nullable(!"1".equals(col.getStr("nullableFlag"))).colComment(col.getStr("columnComment")).build());
            }
        }
        dto.setColumnsList(dbColumns);
        if (AssetsDatasourceService.creaDatasourceTeTable(dto)) {
            return AjaxResult.success("创建成功");
        }
        return AjaxResult.error("表已存在");
    }
}
