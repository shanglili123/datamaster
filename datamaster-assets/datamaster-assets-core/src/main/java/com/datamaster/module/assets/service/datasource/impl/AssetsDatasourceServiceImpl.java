...
// 更新数据源
    ...
    private void replaceDatasourceSpaceRel(AssetsDatasourceDO AssetsDatasourceDO) {
        ...
        if (CollectionUtils.isNotEmpty(AssetsDatasourceDO.getSpaceList())) {
            List<AssetsDatasourceSpaceRelDO> spaceRelList = AssetsDatasourceDO.getSpaceList().stream()
                    .map(spaceRel -> BeanUtils.toBean(spaceRel, AssetsDatasourceSpaceRelDO.class))
                    .collect(Collectors.toList());
            for (AssetsDatasourceSpaceRelDO spaceRel : spaceRelList) {
                spaceRel.setDatasourceId(AssetsDatasourceDO.getId());
                spaceRel.setId(null);
            }
            assetsDatasourceSpaceRelService.saveBatch(spaceRelList);
        }
    }
...
// 批量删除数据源
    ...
    public String getSpaceCodeByDatasourceId(Long datasourceId) {
        AssetsDatasourceSpaceRelDO query = new AssetsDatasourceSpaceRelDO();
        query.setDatasourceId(datasourceId);
        List<AssetsDatasourceSpaceRelDO> relList = assetsDatasourceSpaceRelService.getDatasourceSpaceRelList(query);
        if (relList.isEmpty()) {
            throw new ServiceException("数据源未关联空间");
        }
        return relList.get(0).getSpaceCode();
    }
    ...
    public AssetsDatasourceDO getDatasourceDOById(Long id) {
        ...
        List<AssetsDatasourceSpaceRelDO> assetsDatasourceSpaceRelList = assetsDatasourceSpaceRelService.getJoinSpaceAndDatasource(AssetsDatasourceSpaceRelDO);
        AssetsDatasourceDO.setSpaceList(assetsDatasourceSpaceRelList.stream()
                .map(spaceRel -> (DatasourceSpaceRelDO) spaceRel)
                .collect(Collectors.toList()));
...
// cnName 映射为字段注释
    ...
    @Override
    public PageResult<SpaceRespDTO> getNoDppAddList(SpaceReqDTO pageReqVO) {
        PageResult<SpaceRespDTO> spacePage = taxonomySpaceApi.getSpacePage(pageReqVO);
        ...
        for (Object row : spacePage.getRows()) {
            SpaceRespDTO spaceRespDTO = (SpaceRespDTO) row;
