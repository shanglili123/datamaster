...
public interface AssetsAssetColumnMapper extends BaseMapperX<AssetsAssetColumnDO> {
    ...
    /**
     * 元数据同步专用：仅更新字段元数据（名称/注释/类型/长度/精度/可空/主键/默认值），
     * 保留 DATA_ELEM_CODE_ID、SENSITIVE_LEVEL_ID 等扩展关联字段。
     */
    int syncUpdateColumnMetadata(AssetsAssetColumnDO AssetsAssetColumnDO);
