...
import com.datamaster.common.database.core.*;
import com.datamaster.common.database.datasource.AbstractDataSourceFactory;
import com.datamaster.common.database.datasource.DefaultDataSourceFactoryBean;
import com.datamaster.common.database.dialect.FileDialect;
import com.datamaster.common.database.dialect.OracleDialect;
...
public abstract class AbstractDbQueryFactory implements DbQuery {
    ...
    public void close() {
        ...
        if (DbType.KAFKA.getDb().equals(dbType.getDb())
                ...
                || DbType.MONGODB.getDb().equals(dbType.getDb())
                || DbType.ELASTICSEARCH.getDb().equals(dbType.getDb())) {
            return;
    ...
    public List<DbColumn> getTableColumns(String dbName, String tableName) {
        ...
        return jdbcTemplate.query(sql, dbDialect.columnMapper()).stream()
    ...
    public List<DbColumn> getDbColumns(DbQueryProperty dbQueryProperty) {
        if (DbType.MONGODB.getDb().equals(dbQueryProperty.getDbType())) {
            List<DbColumn> allColumns = new ArrayList<>();
            for (DbTable table : getMongoTables(dbQueryProperty)) {
                allColumns.addAll(getMongoColumns(dbQueryProperty, table.getTableName()));
            }
            return allColumns;
        }
        if (DbType.ELASTICSEARCH.getDb().equals(dbQueryProperty.getDbType())) {
            List<DbColumn> allColumns = new ArrayList<>();
            for (DbTable table : getElasticsearchTables(dbQueryProperty)) {
                allColumns.addAll(getElasticsearchColumns(dbQueryProperty, table.getTableName()));
            }
            return allColumns;
        }
        if (DbType.HIVE.getDb().equals(dbQueryProperty.getDbType())) {
            List<DbColumn> allColumns = new ArrayList<>();
            for (DbTable table : getTables(dbQueryProperty)) {
                List<DbColumn> columns = getTableColumns(dbQueryProperty, table.getTableName());
    ...
    public List<DbColumn> getTableColumns(DbQueryProperty dbQueryProperty, String tableName) {
        if (DbType.MONGODB.getDb().equals(dbQueryProperty.getDbType())) {
            return getMongoColumns(dbQueryProperty, tableName);
        }
        if (DbType.ELASTICSEARCH.getDb().equals(dbQueryProperty.getDbType())) {
            return getElasticsearchColumns(dbQueryProperty, tableName);
        }
        ...
        List<DbColumn> query = jdbcTemplate.query(sql, dbDialect.columnMapper()).stream()
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
        if (CollectionUtils.isEmpty(query)) {
            return query;
...
//        DbDialect otherDialect = DialectFactory.getDialect(DbType.getDbType(dbQueryProperty.getDbType()));
    ...
    public List<DbTable> getTables(DbQueryProperty dbQueryProperty) {
        if (DbType.MONGODB.getDb().equals(dbQueryProperty.getDbType())) {
            return getMongoTables(dbQueryProperty);
        }
        if (DbType.ELASTICSEARCH.getDb().equals(dbQueryProperty.getDbType())) {
            return getElasticsearchTables(dbQueryProperty);
        }
        if (DbType.HIVE.getDb().equals(dbQueryProperty.getDbType())) {
    ...
    public int countNew(String sql, Map<String, Object> params) {
        if (DbType.MONGODB.getDb().equals(dbQueryProperty.getDbType())) {
            try (MongoClient client = createMongoClient(dbQueryProperty)) {
                return (int) client.getDatabase(dbQueryProperty.getDbName()).getCollection(sql).countDocuments();
            }
        }
        if (DbType.ELASTICSEARCH.getDb().equals(dbQueryProperty.getDbType())) {
            return getElasticsearchCount(dbQueryProperty, sql);
        }
        NamedParameterJdbcTemplate namedJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
    ...
    public int countNew(String tableName, DbQueryProperty dbQueryProperty, String where) {
        if (DbType.MONGODB.getDb().equals(dbQueryProperty.getDbType())) {
            try (MongoClient client = createMongoClient(dbQueryProperty)) {
                return (int) client.getDatabase(dbQueryProperty.getDbName()).getCollection(tableName).countDocuments();
            }
        }
        if (DbType.ELASTICSEARCH.getDb().equals(dbQueryProperty.getDbType())) {
            return getElasticsearchCount(dbQueryProperty, tableName);
        }
    ...
    public List<Map<String, Object>> queryDbColumnByList(List<DbColumn> columns, String tableName, DbQueryProperty dbQueryProperty, String where, List<Map> orderByList, long offset, long size) {
        ...
        if (CollectionUtils.isNotEmpty(orderByList)) {
            ...
            for (int i = 0; i < orderByList.size(); i++) {
                ...
                if (com.datamaster.common.utils.StringUtils.isNotBlank(orderByColumn)) {
                    // 拼接表名 + 字段
...
//            this.dbDialect.get
    ...
    private MongoClient createMongoClient(DbQueryProperty property) {
        return MongoClients.create(MongoClientSettings.builder()
                .applyConnectionString(new ConnectionString(property.trainToJdbcUrl()))
                .build());
    }

    private List<DbTable> getMongoTables(DbQueryProperty property) {
    ...
    private List<DbColumn> getMongoColumns(DbQueryProperty property, String collectionName) {
        List<DbColumn> columns = new ArrayList<>();
        Map<String, DbColumn> columnMap = new LinkedHashMap<>();
        try (MongoClient client = createMongoClient(property)) {
            MongoCollection<Document> collection = client.getDatabase(property.getDbName()).getCollection(collectionName);
            int sampled = 0;
            for (Document document : collection.find().limit(100)) {
                flattenMongoDocument(collectionName, "", document, columnMap);
                sampled++;
            }
            if (sampled == 0) {
                DbColumn id = buildMetadataColumn(collectionName, "_id", "OBJECT_ID", 1);
                id.setColKey(true);
                columnMap.put("_id", id);
            }
        }
        int position = 1;
        for (DbColumn column : columnMap.values()) {
            column.setColPosition(position++);
            columns.add(column);
        }
        return columns;
    }

    private void flattenMongoDocument(String tableName, String prefix, Document document, Map<String, DbColumn> columnMap) {
        for (Map.Entry<String, Object> entry : document.entrySet()) {
            String name = prefix.isEmpty() ? entry.getKey() : prefix + "." + entry.getKey();
            Object value = entry.getValue();
            if (value instanceof Document) {
                flattenMongoDocument(tableName, name, (Document) value, columnMap);
                continue;
            }
            if (!columnMap.containsKey(name)) {
                DbColumn column = buildMetadataColumn(tableName, name, mongoType(value), columnMap.size() + 1);
                column.setColKey("_id".equals(name));
                columnMap.put(name, column);
            }
        }
    }

    private String mongoType(Object value) {
        if (value == null) return "NULL";
        if (value instanceof Integer) return "INT";
        if (value instanceof Long) return "BIGINT";
        if (value instanceof Double || value instanceof Float || value instanceof BigDecimal) return "DOUBLE";
        if (value instanceof Boolean) return "BOOLEAN";
        if (value instanceof java.util.Date) return "DATE";
        if (value instanceof List) return "ARRAY";
        if (value instanceof Document || value instanceof Map) return "OBJECT";
        return value.getClass().getSimpleName().toUpperCase();
    }

    private List<DbTable> getElasticsearchTables(DbQueryProperty property) {
        List<DbTable> tables = new ArrayList<>();
        JSONArray indices = JSON.parseArray(esRequest(property, "/_cat/indices?format=json&h=index"));
        for (int i = 0; i < indices.size(); i++) {
            JSONObject item = indices.getJSONObject(i);
            String index = item.getString("index");
            if (org.apache.commons.lang3.StringUtils.isBlank(index) || index.startsWith(".")) {
                continue;
            }
            DbTable table = new DbTable();
    ...
    private List<DbColumn> getElasticsearchColumns(DbQueryProperty property, String index) {
        JSONObject mapping = JSON.parseObject(esRequest(property, "/" + index + "/_mapping"));
        JSONObject indexObj = mapping.getJSONObject(index);
        JSONObject mappings = indexObj == null ? null : indexObj.getJSONObject("mappings");
        JSONObject properties = mappings == null ? null : mappings.getJSONObject("properties");
        Map<String, DbColumn> columnMap = new LinkedHashMap<>();
        flattenElasticProperties(index, "", properties, columnMap);
        return new ArrayList<>(columnMap.values());
    }

    private void flattenElasticProperties(String tableName, String prefix, JSONObject properties, Map<String, DbColumn> columnMap) {
        if (properties == null) {
            return;
        }
        for (String field : properties.keySet()) {
            JSONObject definition = properties.getJSONObject(field);
            String name = prefix.isEmpty() ? field : prefix + "." + field;
            JSONObject nestedProperties = definition.getJSONObject("properties");
            if (nestedProperties != null && !nestedProperties.isEmpty()) {
                flattenElasticProperties(tableName, name, nestedProperties, columnMap);
            }
            String type = definition.getString("type");
            if (org.apache.commons.lang3.StringUtils.isBlank(type)) {
                type = nestedProperties == null ? "object" : "object";
            }
            DbColumn column = buildMetadataColumn(tableName, name, type.toUpperCase(), columnMap.size() + 1);
            columnMap.put(name, column);
        }
    }

    private int getElasticsearchCount(DbQueryProperty property, String index) {
        JSONObject count = JSON.parseObject(esRequest(property, "/" + index + "/_count"));
        return count.getIntValue("count");
    }
    ...
    private String esRequest(DbQueryProperty property, String path, String method, String body) {
        try {
            ...
            throw new DataQueryException("Elasticsearch元数据请求失败: " + e.getMessage());
        }
    }

    private DbColumn buildMetadataColumn(String tableName, String name, String type, int position) {
