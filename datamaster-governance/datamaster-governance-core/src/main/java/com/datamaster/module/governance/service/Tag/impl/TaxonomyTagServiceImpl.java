...
package com.datamaster.module.governance.service.Tag.impl;
...
import com.datamaster.module.governance.api.Tag.dto.TaxonomyTagRespDTO;
import com.datamaster.module.governance.api.service.cat.tag.ITaxonomyTagApiService;
import com.datamaster.module.governance.controller.admin.tag.vo.TaxonomyTagPageReqVO;
import com.datamaster.module.governance.controller.admin.tag.vo.TaxonomyTagRespVO;
import com.datamaster.module.governance.controller.admin.tag.vo.TaxonomyTagSaveReqVO;
...
import com.datamaster.module.governance.dal.dataobject.Rel.TaxonomyTagAssetRelDO;
import com.datamaster.module.governance.dal.dataobject.Tag.TaxonomyTagDO;
import com.datamaster.module.governance.dal.mapper.Tag.TaxonomyTagMapper;
import com.datamaster.module.governance.service.Rel.ITaxonomyTagAssetRelService;
import com.datamaster.module.governance.service.Tag.ITaxonomyTagService;
...
public class TaxonomyTagServiceImpl extends ServiceImpl<TaxonomyTagMapper, TaxonomyTagDO> implements ITaxonomyTagService, ITaxonomyTagApiService {
    ...
    public Long createAttTag(TaxonomyTagSaveReqVO createReqVO) {
        ...
        List<TaxonomyCategoryDO> tagCats = taxonomyCategoryService.getCategoryList(CatType.TAG.getValue());
        TaxonomyCategoryDO tagCat = tagCats.stream()
                .filter(c -> c.getCode().equals(dictType.getCatCode()))
                .findFirst().orElse(null);
        if (tagCat == null) {
            throw new ServiceException("标签类目不存在");
        }
        dictType.setCatName(tagCat.getName());
    ...
    public int updateAttTag(TaxonomyTagSaveReqVO updateReqVO) {
        ...
        if(StringUtils.isNotEmpty(catCode)){
            List<TaxonomyCategoryDO> tagCats = taxonomyCategoryService.getCategoryList(CatType.TAG.getValue());
            TaxonomyCategoryDO tagCat = tagCats.stream()
                    .filter(c -> c.getCode().equals(catCode))
                    .findFirst().orElse(null);
            if (tagCat == null) {
                throw new ServiceException("标签类目不存在");
            }
            updateObj.setCatName(tagCat.getName());
        }
