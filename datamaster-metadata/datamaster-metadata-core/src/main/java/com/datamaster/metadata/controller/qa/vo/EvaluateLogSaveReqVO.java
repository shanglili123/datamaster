...
@Data
@NoArgsConstructor
public class EvaluateLogSaveReqVO extends BaseEntity {

    private static final long serialVersionUID = 1L;

    public EvaluateLogSaveReqVO(QualityRuleEntity rule) {
        ...
        if (rule.getEvaluateId() != null) {
            this.evaluateId = String.valueOf(rule.getEvaluateId());
        }
        if (rule.getTaskLogId() != null) {
            this.taskLogId = String.valueOf(rule.getTaskLogId());
        }
        this.checkDate = new Date();
        if (rule.getConfig() != null) {
            this.rule = JSON.toJSONString(rule.getConfig());
        }
    }
    ...
    List<EvaluateLogStatisticsVO> EvaluateLogStatisticsVOList;
