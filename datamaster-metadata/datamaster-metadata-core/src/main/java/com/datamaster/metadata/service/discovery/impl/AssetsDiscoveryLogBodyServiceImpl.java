...
import com.datamaster.metadata.dal.dataobject.discovery.AssetsDiscoveryLogBodyDO;
import com.datamaster.metadata.dal.mapper.discovery.AssetsDiscoveryLogBodyMapper;
import com.datamaster.metadata.service.discovery.IAssetsDiscoveryLogBodyService;
...
public class AssetsDiscoveryLogBodyServiceImpl extends ServiceImpl<AssetsDiscoveryLogBodyMapper, AssetsDiscoveryLogBodyDO> implements IAssetsDiscoveryLogBodyService {
    // 可放到常量类，这里内联
    ...
    @Resource
    private AssetsDiscoveryLogBodyMapper AssetsDiscoveryLogBodyMapper;
    @Resource
    @Lazy
    private IRedisService redisService;

    @Override
    public int saveOrUpdateByPk(AssetsDiscoveryLogBodyDO entity) {
        AssetsDiscoveryLogBodyDO old = this.getOne(Wrappers.lambdaQuery(AssetsDiscoveryLogBodyDO.class).eq(AssetsDiscoveryLogBodyDO::getTaskId, entity.getTaskId()));
        if (old != null) {
            ...
            return AssetsDiscoveryLogBodyMapper.update(old, Wrappers.lambdaUpdate(AssetsDiscoveryLogBodyDO.class).eq(AssetsDiscoveryLogBodyDO::getTaskId, entity.getTaskId()));
        } else {
            return AssetsDiscoveryLogBodyMapper.insert(entity);
        }
    }

    @Override
    public String getLatestLog(Long taskId) {
        Page<AssetsDiscoveryLogBodyDO> page = this.page(new Page<>(1, 1), Wrappers.lambdaQuery(AssetsDiscoveryLogBodyDO.class).eq(AssetsDiscoveryLogBodyDO::getTaskId, taskId).orderByDesc(AssetsDiscoveryLogBodyDO::getTm));
    ...
    public String getLog(Long taskId) {
        AssetsDiscoveryLogBodyDO row = this.getOne(Wrappers.lambdaQuery(AssetsDiscoveryLogBodyDO.class).eq(AssetsDiscoveryLogBodyDO::getTaskId, taskId));
...
// 重置并短期缓存，便于前端“会话结束后”仍可查询
    ...
    public boolean deleteByPk(Long taskId) {
        ...
        return this.remove(Wrappers.lambdaQuery(AssetsDiscoveryLogBodyDO.class).eq(AssetsDiscoveryLogBodyDO::getTaskId, taskId));
    }
}
