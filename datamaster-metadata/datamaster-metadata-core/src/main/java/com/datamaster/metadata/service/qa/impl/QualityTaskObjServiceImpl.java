...
public class QualityTaskObjServiceImpl  extends ServiceImpl<QualityTaskObjMapper,QualityTaskObjDO> implements IQualityTaskObjService {
    ...
    @Override
    public List<QualityTaskObjDO> getQualityTaskObjList(QualityTaskObjPageReqVO pageReqVO) {
        LambdaQueryWrapperX<QualityTaskObjDO> queryWrapperX = new LambdaQueryWrapperX<>();
        queryWrapperX.eq(pageReqVO.getTaskId() != null,QualityTaskObjDO::getTaskId, pageReqVO.getTaskId());
        List<QualityTaskObjDO> QualityTaskObjDOS = QualityTaskObjMapper.selectList(queryWrapperX);
        return QualityTaskObjDOS;
    }
                ...
                if (isUpdateSupport) {
                    if (QualityTaskObjId != null) {
                        ...
                        if (existingQualityTaskObj != null) {
                            ...
                            successNum++;
                            successMessages.add("数据更新成功，ID为 " + QualityTaskObjId + " 的质量探查任务-稽查对象记录。");
                        } else {
                            failureNum++;
                            failureMessages.add("数据更新失败，ID为 " + QualityTaskObjId + " 的质量探查任务-稽查对象记录不存在。");
                        }
                    } else {
                        failureNum++;
                    ...
                    if (existingQualityTaskObj == null) {
                        ...
                        successNum++;
                        successMessages.add("数据插入成功，ID为 " + QualityTaskObjId + " 的质量探查任务-稽查对象记录。");
                    } else {
                        failureNum++;
                        failureMessages.add("数据插入失败，ID为 " + QualityTaskObjId + " 的质量探查任务-稽查对象记录已存在。");
                    }
                }
            } catch (Exception e) {
                failureNum++;
