...
import com.datamaster.metadata.api.service.task.ICatalogTaskApiService;
import com.datamaster.metadata.controller.admin.metadata.vo.*;
...
import com.datamaster.metadata.dal.dataobject.metadata.CatalogColumnDO;
import com.datamaster.metadata.dal.dataobject.metadata.CatalogDbDO;
import com.datamaster.metadata.dal.dataobject.metadata.CatalogTableDO;
...
import com.datamaster.metadata.dal.dataobject.task.CatalogTaskSchedulerDO;
import com.datamaster.metadata.dal.dataobject.task.CatalogTaskScopeDO;
import com.datamaster.metadata.dal.mapper.metadata.CatalogColumnMapper;
import com.datamaster.metadata.dal.mapper.metadata.CatalogDbMapper;
import com.datamaster.metadata.dal.mapper.metadata.CatalogTableMapper;
...
import com.datamaster.metadata.dal.mapper.task.CatalogTaskMapper;
import com.datamaster.metadata.enums.CollectionScopeEnum;
import com.datamaster.metadata.enums.SchedulerStatusEnum;
import com.datamaster.metadata.service.columnLog.ICatalogColumnLogService;
...
import com.datamaster.metadata.service.scheduler.CatalogTaskDolphinSchedulerService;
import com.datamaster.metadata.service.quality.QualityTaskExecutorService;
import com.datamaster.metadata.service.tableColumnRelLog.ICatalogTableColumnRelLogService;
import com.datamaster.metadata.service.task.ICatalogTaskQualityService;
import com.datamaster.metadata.service.tableLog.ICatalogTableLogService;
import com.datamaster.metadata.service.task.*;
...
public class CatalogTaskServiceImpl extends ServiceImpl<CatalogTaskMapper, CatalogTaskDO> implements ICatalogTaskService, ICatalogTaskApiService {
    ...
    @Resource
    private ISourceSystemApi attSourceSystemApiService;
    @Resource
    ...
    public Long createCatalogTask(CatalogTaskSaveReqVO createReqVO) {
        ...
        // 级联保存质量规则
        saveQualityRules(id, createReqVO, dictType.getSpaceId(), dictType.getSpaceCode());
    ...
    public int updateCatalogTask(CatalogTaskSaveReqVO updateReqVO) {
        ...
        // 级联更新质量规则
        saveQualityRules(updateObj.getId(), updateReqVO, updateObj.getSpaceId(), updateObj.getSpaceCode());
    ...
    public int removeCatalogTask(Collection<Long> idList) {
        ...
        // 删除关联的质量探查任务绑定
        catalogTaskQualityService.removeByCatTaskIds(idList);
        // 批量删除采集任务
    ...
    public CatalogTaskRespVO getCatalogTaskByIdNew(Long id) {
        ...
        DatasourceRespDTO CatalogDatasourceById = daDatasourceApiService.getDatasourceById(bean.getDatasourceId());
    ...
    public boolean runDaDiscoveryTask(Long taskId) {
        ...
        try {
            ...
            triggerLinkedQualityTasks(task);
            return true;
        } catch (Exception e) {
    ...
    /**
     * 探查任务执行联动：元数据采集成功后，触发绑定的质量探查任务执行
     */
    private void triggerLinkedQualityTasks(CatalogTaskRespVO task) {
        if (task == null || task.getId() == null) {
            return;
        }
        try {
            List<Long> qualityTaskIds = catalogTaskQualityService.getQualityTaskIdListByCatTaskId(task.getId());
            if (CollectionUtils.isEmpty(qualityTaskIds)) {
                return;
            }
            log.info("探查任务执行联动，catTaskId={}，绑定质量任务={}", task.getId(), qualityTaskIds);
            for (Long qualityTaskId : qualityTaskIds) {
                try {
                    qualityTaskExecutorService.executeTask(String.valueOf(qualityTaskId));
                } catch (Exception e) {
                    log.warn("探查任务联动质量任务执行失败，catTaskId={}，qualityTaskId={}", task.getId(), qualityTaskId, e);
                }
            }
        } catch (Exception e) {
            log.warn("探查任务联动质量任务异常，catTaskId={}", task.getId(), e);
        }
    }
...
//        Long CatalogTaskInstance = CatalogTaskInstanceService.createCatalogTaskInstance(instance);
    ...
    private DatasourceRespDTO prepareDatasource(CatalogTaskRespVO task, CatalogTaskInstanceDO instance) {
    ...
    private TableProcessResult executeTaskSafely(CatalogTaskRespVO task, CatalogTaskInstanceDO instance) {
        ...
        DatasourceRespDTO datasource = prepareDatasource(task, instance);
    ...
    private List<CatalogDbRespVO> findDbsOnlyInResp(List<CatalogTaskScopeDO> databaseScopes, DatasourceRespDTO datasource, List<CatalogDbRespVO> CatalogDbByTaskId) {
    ...
    private CatalogDbRespVO findMatchedDb(CatalogDbSaveReqVO dbScope, DatasourceRespDTO datasource, List<CatalogDbRespVO> CatalogDbByTaskId) {
    ...
    private DbQueryContext createDbQueryForScope(DatasourceRespDTO datasource, CatalogDbSaveReqVO dbScope, CatalogTaskRespVO task, CatalogTaskInstanceDO instance) {
    ...
    private TableProcessResult executeSingleDatabase(DbQueryContext dbQuery, CatalogTaskRespVO task, CatalogTaskInstanceDO instance, CatalogDbSaveReqVO dbScope, DatasourceRespDTO datasource) {
        String blacklist = task.getBlacklist();
...
//            Long CatalogTableId = CatalogTableService.createCatalogTable(table);
    ...
    private List<CatalogTaskScopeDO> loadDatabaseScopesFromDatasource(CatalogTaskRespVO task, CatalogTaskInstanceDO instance, DatasourceRespDTO datasource) {
...
//            CatalogTableReqDTO.setId(CatalogTableId);
    ...
    public List<CatalogTaskSourceTreeRespVO> getSourceSystemTree(Long spaceId) {
        // 1. 获取所有有效的来源系统
        List<SourceSystemRespDTO> validSourceSystems = attSourceSystemApiService.getValidSourceSystems(spaceId);
        ...
        List<DatasourceRespDTO> daDatasourceRespDTOList = Lists.newArrayList();
        ...
        Map<Long, DatasourceRespDTO> daDatasourceRespDTOMap = daDatasourceRespDTOList
                .stream()
                .collect(Collectors.toMap(DatasourceRespDTO::getId, daDatasourceRespDTO -> daDatasourceRespDTO));

        List<CatalogTaskSourceTreeRespVO> treeList = Lists.newArrayList();

        // 4. 构建一级节点：来源系统
        for (SourceSystemRespDTO sourceSystem : validSourceSystems) {
            ...
            for (Map.Entry<Long, List<CatalogTaskDO>> dsEntry : datasourceGroupMap.entrySet()) {
                ...
                DatasourceRespDTO daDatasourceRespDTO = daDatasourceRespDTOMap.get(datasourceId);
    ...
    /**
     * 级联保存质量规则：如果探查任务带有质量规则，自动创建/更新对应的 COL_QUALITY_TASK 并关联
     */
    private void saveQualityRules(Long catTaskId, CatalogTaskSaveReqVO reqVO, Long spaceId, String spaceCode) {
        List<QualityTaskObjSaveReqVO> qualityObjs = reqVO.getQualityObjs();
        List<QualityTaskEvaluateSaveReqVO> qualityEvaluates = reqVO.getQualityEvaluates();

        // 没有质量规则数据，清除已有关联
        if (CollectionUtils.isEmpty(qualityObjs) && CollectionUtils.isEmpty(qualityEvaluates)) {
            // 删除已有关联
            List<Long> oldQualityTaskIds = catalogTaskQualityService.getQualityTaskIdListByCatTaskId(catTaskId);
            if (CollectionUtils.isNotEmpty(oldQualityTaskIds)) {
                for (Long qtId : oldQualityTaskIds) {
                    qualityTaskObjService.remove(Wrappers.lambdaQuery(QualityTaskObjDO.class)
                            .eq(QualityTaskObjDO::getTaskId, qtId));
                    qualityTaskEvaluateService.remove(Wrappers.lambdaQuery(QualityTaskEvaluateDO.class)
                            .eq(QualityTaskEvaluateDO::getTaskId, qtId));
                    qualityTaskMapper.deleteById(qtId);
                }
                catalogTaskQualityService.removeByCatTaskIds(Collections.singletonList(catTaskId));
            }
            return;
        }

        // 查找或创建对应的 COL_QUALITY_TASK
        List<Long> existingQtIds = catalogTaskQualityService.getQualityTaskIdListByCatTaskId(catTaskId);
        Long qualityTaskId;
        if (CollectionUtils.isNotEmpty(existingQtIds)) {
            qualityTaskId = existingQtIds.get(0);
        } else {
            // 创建一个新的 COL_QUALITY_TASK 记录
            QualityTaskDO qt = new QualityTaskDO();
            qt.setTaskName(reqVO.getName() + "-质量规则");
            qt.setContact(reqVO.getLeader() != null ? reqVO.getLeader().toString() : null);
            qt.setStatus("0");
            qt.setPriority("0");
            qt.setWorkerGroup("default");
            qt.setRetryTimes(0L);
            qt.setRetryInterval(0L);
            qt.setDelayTime(0L);
            qt.setAssetFlag("0");
            ...
            // 建立关联
            catalogTaskQualityService.bindQualityTask(catTaskId, qualityTaskId);
        }

        // 先清除旧的对象和规则
        qualityTaskObjService.remove(Wrappers.lambdaQuery(QualityTaskObjDO.class)
                .eq(QualityTaskObjDO::getTaskId, qualityTaskId));
        qualityTaskEvaluateService.remove(Wrappers.lambdaQuery(QualityTaskEvaluateDO.class)
                .eq(QualityTaskEvaluateDO::getTaskId, qualityTaskId));

        // 保存稽查对象
        if (CollectionUtils.isNotEmpty(qualityObjs)) {
            for (QualityTaskObjSaveReqVO objReq : qualityObjs) {
                objReq.setId(null);
                objReq.setTaskId(qualityTaskId);
                qualityTaskObjService.createQualityTaskObj(objReq);
            }
        }

        // 保存评测规则，关联稽查对象
        if (CollectionUtils.isNotEmpty(qualityEvaluates)) {
            // 构建 datasourceId+tableName -> objId 的映射
            Map<String, Long> objMap = new HashMap<>();
            if (CollectionUtils.isNotEmpty(qualityObjs)) {
                for (QualityTaskObjSaveReqVO obj : qualityObjs) {
                    String key = obj.getDatasourceId() + "_" + (obj.getTableName() == null ? "" : obj.getTableName());
                    objMap.put(key, obj.getId());
                }
            }
     ...
     * 加载质量规则到响应 VO
     */
    private void loadQualityRules(CatalogTaskRespVO respVO) {
        List<Long> qualityTaskIds = catalogTaskQualityService.getQualityTaskIdListByCatTaskId(respVO.getId());
        if (CollectionUtils.isEmpty(qualityTaskIds)) {
            return;
        }
        Long qualityTaskId = qualityTaskIds.get(0);

        // 加载稽查对象
        List<QualityTaskObjDO> objs = qualityTaskObjService.list(Wrappers.lambdaQuery(QualityTaskObjDO.class)
                .eq(QualityTaskObjDO::getTaskId, qualityTaskId));
        if (CollectionUtils.isNotEmpty(objs)) {
            respVO.setQualityObjs(BeanUtils.toBean(objs, QualityTaskObjRespVO.class));
        }

        // 加载评测规则
        List<QualityTaskEvaluateDO> evals = qualityTaskEvaluateService.list(Wrappers.lambdaQuery(QualityTaskEvaluateDO.class)
                .eq(QualityTaskEvaluateDO::getTaskId, qualityTaskId));
        if (CollectionUtils.isNotEmpty(evals)) {
            respVO.setQualityEvaluates(BeanUtils.toBean(evals, QualityTaskEvaluateRespVO.class));
        }
    }
}
