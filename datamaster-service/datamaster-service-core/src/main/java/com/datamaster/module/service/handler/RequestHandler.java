...
import com.datamaster.module.service.utils.DataTimeUtil;
...
//    SysUserServiceFeign sysUserService;
    ...
    public Object invoke(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            // 序列化
            api = objectMapper.readValue(objectMapper.writeValueAsString(api), ServiceApiDO.class);
            // 执行前置拦截器
            requestInterceptor.preHandle(request, response, api, params);

            //创建返回值
            Object responseValuel;
            //判断是什么类型的接口请求，1-数据服务。2-模型数据服务，4-文件服务
            String isIntegrate = api.getApiServiceType();
            if (StringUtils.equals("4", isIntegrate)) {//文件服务
                //返回文件
            ...
            //创建日志实体
            ServiceApiLogDO apiLogDto = new ServiceApiLogDO();
