<template>
  <div class="app-container" ref="app-container">
    ...
    <a-layout style="90%">
      <DeptTree :deptOptions="deptOptions" :leftWidth="leftWidth" :placeholder="'请输入数据元目录'"
...
>
            <a-form-item label="中文" name="name">
              <a-input style="width: 160px" v-model:value="queryParams.name" placeholder="请输入中文名称" allow-clear
                @pressEnter="handleQuery"
/>
            </a-form-item>
            <a-form-item label="英文" name="engName">
              <a-input style="width: 160px" v-model:value="queryParams.engName" placeholder="请输入英文名称" allow-clear
                @pressEnter="handleQuery"
/>
            </a-form-item>
            <a-form-item label="类型" name="type">
              <a-select style="width: 160px" v-model:value="queryParams.type" placeholder="请选择类型">
                <a-select-option v-for="dict in dp_data_elem_code_type" :key="dict.value" :label="dict.label"
                  :value="dict.value"
>{{ dict.label }}</a-select-option>
          ...
          <div class="data-action-btns">
            <a-button type="primary" @click="handleAdd" v-hasPermi="['dp:dataElem:add']"
...
>修改</a-button>
                <a-button type="link" danger size="small" @click="handleDelete(record)" :disabled="record.status === '1'"
          ...
          <a-col :span="12">
            <a-form-item label="数据元目录" name="catCode">
              <a-tree-select show-search v-model:value="form.catCode" :tree-data="deptOptions"
                :field-names="{ value: 'code', label: 'name', children: 'children' }" placeholder="请选择数据元目录"
                tree-check-strictly
/>
            </a-form-item>
          </a-col>
        ...
        <a-row :gutter="20">
          <a-col :span="12">
            <a-form-item label="标准类型" name="description">
              <a-select class="el-form-input-width" v-model:value="form.documentType" placeholder="请选择类型" allow-clear
                @change="fetchSecondLevelDocs" style="width: 100%;"
>
                <a-select-option v-for="dict in dp_document_type" :key="dict.value" :label="dict.label"
                  :value="dict.value"
>{{ dict.label }}</a-select-option>
              </a-select>
            </a-form-item>
          ...
          <a-col :span="12">
            <a-form-item label="标准登记" name="documentId">
              <a-select class="el-form-input-width" v-model:value="form.documentId" placeholder="请选择标准进行绑定"
                style="width: 100%;"
>
                <a-select-option v-for="doc in secondLevelDocs" :key="doc.value" :label="doc.label" :value="doc.value">{{ doc.label }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
        </a-row>
          ...
          <a-col :span="12">
            <a-form-item label="类型" name="type">
              <a-radio-group v-model:value="form.type" :disabled="form.id">
                <a-radio v-for="dict in dp_data_elem_code_type" :key="dict.value" :value="dict.value">{{ dict.label }}
                </a-radio>
              </a-radio-group>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="状态" name="status">
              <a-radio-group v-model:value="form.status">
                <a-radio v-for="dict in sys_disable" :key="dict.value" :value="dict.value">{{ dict.label }}
                </a-radio>
              </a-radio-group>
            </a-form-item>
          </a-col>
        ...
        <a-row :gutter="20">
          <a-col :span="24">
            <a-form-item label="备注">
              <a-input type="textarea" placeholder="请输入备注" v-model:value="form.remark" :min-height="192" />
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
...
>
      <a-upload ref="uploadRef" :max-count="1" accept=".xlsx, .xls" :headers="upload.headers"
        ...
        :before-upload="handleFileUploadProgress" :auto-upload="false" drag @change="handleUploadChange"
>
...
const tableColumns = computed(() => {
  const allCols = [
    ...
    { title: '类型', dataIndex: 'type', align: 'center', width: 100, colKey: 3 },
    { title: '数据元目录', dataIndex: 'catCode', align: 'left', width: 120, ellipsis: true, colKey: 4 },
    { title: '创建人', dataIndex: 'createBy', align: 'left', width: 140, ellipsis: true, colKey: 10 },
