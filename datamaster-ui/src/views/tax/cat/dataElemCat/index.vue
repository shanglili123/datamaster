<template>
  <div class="app-container" ref="app-container">
    ...
    <PageHeader
      ...
      :showAddBtn="true"
      :addPermission="['tax:dataElemCat:add']"
      :showToggleBtn="true"
    ...
    >
      ...
      <template #searchForm>
        <a-form-item label="数据元目录名称" name="name">
          <a-input
            class="el-form-input-width"
            v-model:value="queryParams.name"
            placeholder="请输入数据元目录名称"
            allow-clear
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'actions'">
            <a-button
              ...
              v-hasPermi="['tax:dataElemCat:edit']"
              >修改</a-button
            >
            <a-button
              type="link"
              ...
              v-hasPermi="['tax:dataElemCat:add']"
              >新增</a-button
            >
            <a-button
              type="link"
              ...
              v-hasPermi="['tax:dataElemCat:remove']"
              >删除</a-button
            >
          </template>
        </template>
...
const tableColumns = [
  { title: "数据元目录名称", dataIndex: "name", align: "left", width: 200, ellipsis: true },
...
const data = reactive({
  ...
  rules: {
    name: [
      { required: true, message: "数据元目录名称不能为空", trigger: "blur" },
    ],
    parentId: [
...
function handleStatusChange(row) {
  ...
  proxy.$modal
    .confirm(`确认要"${text}","${row.name}"数据元目录吗？`)
    .then(() => {
...
function handleDelete(row) {
  proxy.$modal
    .confirm('是否确认删除数据元目录管理编号为"' + row.name + '"的数据项？')
    .then(function () {
