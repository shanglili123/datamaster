<template>
  <div class="app-container" ref="app-container">
    ...
    >
      ...
      <template #searchForm>
        <a-form-item label="质量探查目录名称" name="name" :label-col="{ style: { width: '130px' } }">
          <a-input class="el-form-input-width" v-model:value="queryParams.name" placeholder="请输入质量探查目录名称" allow-clear
            @pressEnter="handleQuery" />
        </a-form-item>
        <a-form-item label="上级目录" name="code">
          <a-tree-select show-search allow-clear class="el-form-input-width" v-model:value="queryParams.code" :tree-data="attAssetCatOptions"
    ...
    <div class="pagecont-bottom">
      <a-table v-if="refreshTable"
        :loading="loading"
        :data-source="attAssetCatList"
        :columns="[
          { title: '质量探查目录名称', dataIndex: 'name', align: 'left', width: 200, ellipsis: true },
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'actions'">
            <a-button type="link" size="small" @click="handleUpdate(record)"
              v-hasPermi="['tax:qualityCat:edit']">修改</a-button>
            <a-button type="link" size="small" @click="handleAdd(record)"
              v-hasPermi="['tax:qualityCat:add']">新增</a-button>
            <a-button type="link" danger size="small" @click="handleDelete(record)"
              v-hasPermi="['tax:qualityCat:remove']">删除</a-button>
          </template>
        </template>
      </a-table>
        ...
        <a-row :gutter="20">
          <a-col :span="12">
            <a-form-item label="目录名称" name="name">
              <a-input v-model:value="form.name" placeholder="请输入质量探查目录名称" />
            </a-form-item>
          </a-col>
          ...
          <a-col :span="12">
            <a-form-item label="上级目录" name="parentId">
              <a-tree-select show-search allow-clear :disabled="form.id" v-model:value="form.parentId" :tree-data="attAssetCatOptions"
...
const data = reactive({
  ...
  rules: {
    name: [{ required: true, message: '质量探查目录名称不能为空', trigger: 'blur' }],
...
function handleStatusChange(row) {
  ...
  proxy.$modal
    .confirm('确认要"' + text + '","' + row.name + '"质量探查目录吗？')
    .then(function () {
...
function handleDelete(row) {
  proxy.$modal
    .confirm('是否确认删除质量探查目录管理编号为"' + row.name + '"的数据项？')
    .then(function () {
