...
| 23 | Data quality category navigation | Fix blank/wrong page after moving data quality category under Metadata Management > Data Quality. | Done | Updated `datamaster-ui/src/store/system/permission.js` so `质量探查目录` is only nested under the actual `质量探查` parent menu instead of accidentally under `质量探查任务`; normalized the flattened router path to `/cat/quality/qualityCat`; ordered `质量探查目录` as the first child under `质量探查`; updated `datamaster-ui/src/layout/components/Sidebar/SidebarItem.vue` so absolute child paths are not re-prefixed by the parent sidebar path; updated `datamaster-ui/src/layout/components/Sidebar/Link.vue` and `datamaster-ui/src/layout/components/TagsView/index.vue` so clicking the current route re-enters through `/redirect` instead of becoming a no-op; removed a leftover debug banner from `datamaster-ui/src/views/att/cat/qualityCat/index.vue`. | Browser verification on `http://127.0.0.1:82` confirmed the quality menu order is `质量探查目录`, `质量探查任务`, `质量任务日志`; direct navigation to `/cat/quality/qualityCat` keeps that URL, renders the category query form and table, and reports no browser errors. `npx eslint src/layout/components/Sidebar/Link.vue src/layout/components/TagsView/index.vue src/store/system/permission.js` passed; `npm run build:prod` passed with existing warnings. `SidebarItem.vue` and `qualityCat/index.vue` still have pre-existing lint issues unrelated to this fix. |
| 24 | Three-level layout refactor | Redesign the layout system so L1 and L2 menus display on the left sidebar, and L3 menus render as horizontal tabs on the top bar. When no L3 exists, the content area fills the full width. Replace all "类目" display text with "目录". | Done | Updated `datamaster-ui/src/layout/index.vue`: rewrote `getSidebarRoutesForCurrentTopMenu()` to return L3 items (children of current L2) instead of L2 items; added `findCurrentL2()` helper; updated watch to always sync sidebarRouters (empty when no L3); removed sidebarRouters dependency from `sidebarHide` computation—now relies on `route.matched.length`. Updated `datamaster-ui/src/store/system/permission.js`: rewrote `buildSidebarRoutesForCurrentPath()` to return L3 items; added `findMatchingL2()` helper; updated `setTopbarRoutes()` to always set sidebarRouters (even if empty). Bulk-replaced "类目" → "目录" across all .vue and .js files under `datamaster-ui/src/` (60+ files). | Pending — `npm run build:prod` to verify no compilation errors. |
...
| 2026-07-30 | 24 | Redesigned layout to L1+L2 on left sidebar and L3 on top horizontal menu; full-width content when no L3. Replaced all "类目" display text with "目录" across the frontend. |
